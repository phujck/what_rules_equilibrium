...
\begin{figure}[t]
    ...
    \caption{\label{fig:qc_equivalence} \textbf{Quantum--Classical Equivalence.} Left: Renormalised splitting $\omega_{\mathrm{MF}}$ of a qubit coupled to a quantum Ohmic bath ($\alpha$) via transverse coupling $\sigma_x$. Right: The corresponding fourth-order cumulant $\kappa_4$ required for a classical non-Gaussian bath (coupled via $P_0$) to reproduce the exact same splitting. The classical bath ``pays'' for the quantum suppression effect with a specific non-Gaussian cost.}
\end{figure}
