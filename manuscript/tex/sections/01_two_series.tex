...
For a full derivation of the results in this section, see Appendix~\ref{app:commutator_derivation}.

For a Gaussian bath ($K_{2n}=0$ for $n\geq 2$), only the second cumulant survives. But when $[H_S,f]\neq 0$, the interaction-picture coupling $\tilde{f}(\tau)=e^{\tau H_S}fe^{-\tau H_S}$ generates new operators. We expand this time-dependence using the adjoint chain
\begin{equation}
...
\end{equation}
Substituting this expansion into the Gaussian influence functional $\Phi^{(2)} = \frac{1}{2}\iint K(\tau,\sigma)\tilde{f}(\tau)\tilde{f}(\sigma)d\tau d\sigma$ yields the \emph{bilocal influence operator}
\begin{equation}
    \Phi^{(2)} \equiv \Delta = \frac{1}{2}\sum_{n,m=0}^\infty C_{nm}(\beta)\,f_n f_m,
    \label{eq:Delta_commutator_form}
\end{equation}
where the coefficients $C_{nm}$ encode the moments of the bath correlation function:
\begin{equation}
    C_{nm}(\beta) = \frac{1}{n!m!} \int_0^\beta d\tau \int_0^\beta d\sigma\, K(\tau-\sigma)\,\tau^n \sigma^m.
    \label{eq:Cnm_definition}
\end{equation}
The reduced equilibrium operator then takes the factorised form
\begin{equation}
    \rhobar(\beta) = e^{-\beta H_S}\,e^{\Delta},
    \label{eq:rhobar_product_form}
\end{equation}
which defines the HMF via $e^{-\beta \HMF} = e^{-\beta H_S}e^\Delta$. This is a standard Lie-algebraic problem: solving for $Z(X,Y)$ in $e^Z = e^X e^Y$ where $X=-\beta H_S$ and $Y=\Delta$. The solution is given by the Baker--Campbell--Hausdorff (BCH) series. To linear order in the coupling $\Delta$ (which contains the small parameter $\alpha$), the solution is given by the derivative of the exponential map:
\begin{equation}
    \HMF = H_S - \frac{1}{\beta} \frac{-\beta \adf}{1-e^{\beta \adf}}(\Delta) + \mathcal{O}(\Delta^2).
    \label{eq:derivative_exp_map}
\end{equation}
Using the generating function of the Bernoulli numbers, $x/(e^x-1) = \sum_{r=0}^\infty (B_r/r!) x^r$, and noting that $x/(1-e^{-x}) = x + x/(e^x-1)$, we obtain the explicit commutator expansion:
\begin{equation}
    \HMF(\beta) = H_S - \frac{1}{\beta}\sum_{r=0}^\infty \frac{B_r}{r!}(-\beta)^r\,\adf^r(\Delta) + \mathcal{O}(\Delta^2).
    \label{eq:HMF_Bernoulli}
\end{equation}
This series is structurally distinct from the polynomial series. The chain index shifts under the adjoint action via the binomial identity
\begin{equation}
    \adf^r(f_n f_m) = \sum_{k=0}^r\binom{r}{k}f_{n+k}\,f_{m+r-k},
    \label{eq:binomial_adjoint}
\end{equation}
meaning $\HMF$ involves nested commutators of increasing depth. 

The operator content is uniquely controlled by the \emph{adjoint algebra} $\mathfrak{a}_f = \mathrm{span}\{f_0,f_1,f_2,\ldots\}$. If this Lie-generated family closes at finite order $N$ --- meaning $f_N$ is a linear combination of $\{f_0,\ldots,f_{N-1}\}$ --- then $\Delta$ (and thus $\HMF$ to all orders) lives in the finite-dimensional associative algebra generated by products of $\{f_0,\ldots,f_{N-1}\}$.
